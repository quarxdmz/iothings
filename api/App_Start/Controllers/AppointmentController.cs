    ...
    public class AppointmentController : ApiController
    {
        ...
        public IHttpActionResult newAppointment(param_appointment par)
        {
            try
            {
                ...
                custom.IsRequired("user_id", par.user_id.ToString(), 2);
                custom.IsRequired("patient_id", par.patient_id.ToString(), 2);
                custom.IsRequired("specialty_id", par.specialty_id.ToString(), 2);
                custom.IsRequired("appointment_type_id", par.appointment_type_id.ToString(), 2);
                //IsRequired("appointment_status_id", appointment_status_id, 2);
                custom.IsRequired("lat", par.lat.ToString(), 2);
                custom.IsRequired("longi", par.longi.ToString(), 2);
        ...
        {
            custom.IsRequired("user_id", user_id, 1);
            if (custom.haserror)
            {
                return Json(new { data = new string[] { }, message = custom.errmsg, success = false });
            }
            else
            {
                ...
                List<doc_specialty> spec = custom.getSpecialty_all();
                        ...
                        foreach (var i in querydata1)
                        {
                            ...
                            else if (hasProposed) // no doctor_id, but has proposed_doctor
                            {
                                ...
                                foreach (var d in prop_doc)
                                {
                                    ...
                                    d.organization_name = doctor_name.organization_name == null ? "" : doctor_name.organization_name;
                                    d.pecos_certificate = doctor_name.pecos_certification == null ? "" : doctor_name.pecos_certification;
        ...
        {
            custom.IsRequired("appointment_id", appointment_id, 2);
